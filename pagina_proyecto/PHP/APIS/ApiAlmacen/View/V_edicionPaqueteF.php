...
<header>
    ...
    <div class="container-add">
        <h2 class="container__title">Panel de edición</h2>
            <form class="container__form" action="../Controller/C_edicionPaqueteF.php" method="post">
            <input type="hidden" value="<?php echo $fila['IdPaquete'];?>" name="id">
                ...
                <div class="form-row">
                    ...
                    foreach ($filas2->fetch_all(MYSQLI_ASSOC) as $fila2) { //convierte el json devuelto en un array para cada lote
                        ...
                        echo '<option value="'.$fila2['IdLote'] .'">'.$fila2['IdLote']." | ".$fila2['DestinoLote'].'</option>';
                    }
                    ?>
                    </select>
                ...
                <div class="botones-container">
                    ...
                    <a href="V_paquetesF.php" class="boton 11">Volver</a>    
                </div>
                </form>
            </div>
    </form>
...
<script>
    $(document).ready(function(){
            $(".idioma-ingles").click(function(){
                ...
                $("select[name='tipo'] option[value='Frágil 3']").text("Fragile");
                $("select[name='tipo'] option[value='Peligroso 4']").text("Dangerous");
                $("select[name='tipo'] option[value='Inflamable 5']").text("Flammable");
            })
            $(".idioma-espana").click(function(){
                ...
                $(".10").val("Confirmar");
                $(".11").text("Volver");
                $("select[name='tipo'] option[value='Frágil 3']").text("Frágil");
                $("select[name='tipo'] option[value='Peligroso 4']").text("Peligroso");
                $("select[name='tipo'] option[value='Inflamable 5']").text("Inflamable");
            })
})
        </script>
</body>
